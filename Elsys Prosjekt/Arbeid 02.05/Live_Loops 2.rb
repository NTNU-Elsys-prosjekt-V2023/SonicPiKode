live_loop :Tracker do
  ...
  if State_list[0][0] == "0"
    ...
    if next_sec == true && !Replay_klick || State_list[0][2] == "0"
...
def bass_player
  ...
  loop do
    ...
    if State_list[Current_section][11..13] == "001"
      use_synth :sine
      instrument_player(bass_list, 0, 0, 0, 1, 0, 0)
...
def chord_player
  ...
  loop do
    if State_list[Current_section][1..3] == "001"
      use_synth :piano
      instrument_player(chord_list)
    elsif State_list[Current_section][1..3] == "010"
      use_synth :tech_saws
      instrument_player(chord_list, 0, 0, 0, 1, 0.3)
    elsif State_list[Current_section][1..3] == "011"
      #with_fx :reverb, damp: 1 do
      #with_fx :hpf, cutoff: 40 do
      use_synth :kalimba
      instrument_player(chord_list, 0, 0.5, 0, 0.05, 1, 5)
      #end
      #end
...
def instrument_player(play_list, atk = 0, dec = 0, sus = 0, sus_lvl = 1, rel = 1, amp = 1)
  idx = 0
  while idx < play_list.length
    play play_list[idx][0], sustain: play_list[idx][1], release: rel
